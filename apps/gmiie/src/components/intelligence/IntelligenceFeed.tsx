...
function IntelligenceFeedInner({
  articles,
  ...
  // Apply language filter first, then type filter
  const langFiltered =
    activeLang === "all"
      ? articles
      : articles.filter((a) => (a as any).language === activeLang || (activeLang === "en" && !(a as any).language));

  const filtered =
    activeFilter === "all"
      ? langFiltered
      : langFiltered.filter((a) => a.articleType === activeFilter);
      ...
      <div className="flex items-center justify-between mb-5">
        <div className="flex items-center gap-2.5">
          ...
          <span className="text-caption font-mono text-text-muted">
            ...
            {(activeFilter !== "all" || activeLang !== "all") && (
        ...
        <div className="flex items-center gap-1 mb-5 overflow-x-auto pb-1 border-b border-border-subtle">
          {ARTICLE_FILTERS.map((filter) => {
            ...
            const count =
              filter.value === "all"
                ? langFiltered.length
                : langFiltered.filter((a) => a.articleType === filter.value).length;
            return (
              <button
                key={filter.value}
...
export function IntelligenceFeed(props: IntelligenceFeedProps) {
  return (
    <Suspense fallback={<div className="animate-pulse text-text-muted p-4">Loading feed…</div>}>
      <IntelligenceFeedInner {...props} />
    </Suspense>
  );
}
